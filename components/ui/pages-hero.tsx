...
const PagesHero = ({ img, title }:{img: string, title: string}) => {
    return (
        <div className='w-full'>
            <div className='relative h-52 bg-dark_green overflow-hidden md:h-80'>
                ...
                <div className='absolute inset-0 bg-gradient-to-tr from-dark_green/40 to-light_green/70 flex flex-col px-2'>
                    <p className='text-lg mt-auto text-near_white font-extrabold uppercase md:text-2xl'>{title}</p>
                </div>
            </div>
        </div>
    )
